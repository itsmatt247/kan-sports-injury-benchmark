"""
Efficient KAN Implementation (B-spline based)
...
class KANLinear(nn.Module):
    ...
    Args:
        ...
        scale_noise: Scale of noise for initialization
        scale_base: Scale for base weight initialization
        scale_spline: Scale for spline weight initialization
        enable_standalone_scale_spline: Whether to use separate scale for spline
        base_activation: Activation function for base path
        grid_eps: Small value for grid initialization
    ...
    def b_splines(self, x: torch.Tensor):
        """
        Compute the B-spline bases for input x.
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        ...
        Args:
            x: Input tensor of shape (batch_size, in_features)
            y: Target tensor of shape (batch_size, in_features, out_features)
            
        Returns:
            Tensor of shape (out_features, in_features, grid_size + spline_order)
        """
        assert x.dim() == 2 and x.size(1) == self.in_features
...
class KAN(nn.Module):
    """
    Kolmogorov-Arnold Network using B-spline basis functions.
    
    Args:
        layers_hidden: List of layer dimensions [in_features, hidden1, hidden2, ..., out_features]
        grid_size: Number of grid intervals for B-splines
        spline_order: Order of B-spline basis functions
        scale_noise: Scale of noise for initialization
        scale_base: Scale for base weight initialization
